...
### Estructura monorepo (obligatoria)

- `mythr-prism-front/`: frontend operativo actual (Vue 3 + TypeScript).
- `mythr-prism-back/`: backend scaffold para evolucion futura.
- `README.md` y `package.json` en raiz definen la orquestacion de workspace PNPM.

## 2) Principios de arquitectura

### Stack y direccion tecnica

- Frontend en `mythr-prism-front` con Vue 3 + TypeScript estricto (`strict: true`).
- Package manager por defecto: PNPM.
- Enfoque incremental: MVP primero, luego V1/V2 segun `mythr-prism-front/docs/backlog.md`.

### Separacion por capas

- `mythr-prism-front/src/components/`: UI y composicion visual.
- `mythr-prism-front/src/composables/`: logica de dominio/reactividad reusable.
- `mythr-prism-front/src/services/`: infraestructura (persistencia, html de slave, integraciones browser API).
- `mythr-prism-front/src/types/`: contratos de dominio y mensajes.
- `mythr-prism-front/src/utils/`: utilidades puras y acotadas.
- `mythr-prism-front/src/assets/`: recursos estaticos (styles, imagenes, fuentes y otros assets del runtime/UI).
...
- Modelar contratos con `type`/`interface` en `mythr-prism-front/src/types` cuando sean compartidos.
...
- Hoja global principal en `mythr-prism-front/src/assets/styles/style.css`.
...
- Checkboxes inline de formulario deben usar un componente visual unificado y accesible (por defecto `mythr-prism-front/src/components/ui/AppCheckbox.vue`).
...
- Definir y mantener tipos en `mythr-prism-front/src/types/messages.ts`.
...
- `mythr-prism-front/docs/backlog.md` es la fuente oficial de plan y progreso.
...
4. Documentar impacto breve en `mythr-prism-front/docs/backlog.md` si afecta roadmap/estado.

### B) Feature MVP

1. Confirmar alcance minimo viable en `mythr-prism-front/docs/backlog.md`.
...
Este documento aplica a cualquier agente/IA que trabaje en este repositorio. Si una nueva decision de arquitectura contradice alguna regla, actualizar este archivo y `mythr-prism-front/docs/backlog.md` en la misma iteracion para mantener consistencia operativa.
